...
\textbf{Write Analysis:} The writes would be $N_R \times (size_{entry} + size_{ptr})$ since each new entry connects to the previous entry in the bucket by means of a pointer.

\subsection{Page based allocation}
Allocation of space to buckets is done in units of \textit{pages}. A page contains contiguous entries and is connected to another page by means of a pointer. Each time a bucket runs out of space, we allocate a new page to the bucket. Though such an approach  may lead to space wastage when some of the pages are not fully occupied, we save on valuable pointer writes which are incurred when the space is allocated on a per-entry basis.

\textbf{Write Analysis:} Assuming there are $T_p$ entries per page, there would be one pointer per $T_p$ entries. In such a case, the writes would be $N_R \times (size_{entry} + \frac{size_{ptr}}{T_p})$.
