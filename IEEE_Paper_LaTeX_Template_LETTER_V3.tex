...
\item We test our proposed algorithms on TPC-H benchmark queries using the modified simulator. Our results show significant improvements both in running time and writes compared to existing algorithms.
...
a) \textbf{PCM\_DISK}: In this model, PCM is proposed as an alternative to disk. The advantage of this model is that existing database systems can run oblivious to the change in the underlying hardware treating PCM as the disk. However, given the huge database sizes of today, the density and cost of PCM would make it infeasible to accommodate the entire database on PCM itself. Disk, by virtue of being much denser and cheaper, is likely continue to be the memory technology for persistent database storage.
...
We modified Multi2sim \cite{multi2sim} in order to include Phase Change Memory in the memory hierarchy. Multi2sim is an open source application-only simulator that can simulate a multithreaded, multicore, superscalar x86 CPU and an AMD Evergreen GPU. It has support for both functional simulation and cycle accurate detailed architecture simulation.
...
We used TPCH 1GB database for our experiments. The database was disk-resident. We implemented code for full TPC-H query execution. 
...
\begin{tabular}{p{2.3cm}p{5.5cm}}
\toprule
Simulator & Multi2Sim-4.2 enhanced with PCM support\\ \hline

\multirow{2}{*}{CPU Cache} & Private L1D (32KB, 8-way, 4 cycle latency)\\  
& Private L2 (256KB, 8-way, 11 cycle latency)\\ \hline

DRAM & 2MB, 16-way, 100 cycle latency\\ \hline

PCM & read latency: 230 cycles, write latency per 4B modified word: 225 cycles \\ \hline
HD & 2 GB \\
\bottomrule
\end{tabular}
\end{table}
