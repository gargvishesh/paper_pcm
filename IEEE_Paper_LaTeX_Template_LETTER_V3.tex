...
%
\begin{abstract} 
\textit{Phase Change Memory (PCM)} is a non-volatile, random access and byte-addressable memory that is being touted as one of the most promising upcoming memory technologies. Besides its desirable features of high density, comparable read latency, and low idle power vis-a-vis the DRAM, PCM also comes with certain peculiar characteristics of asymmetric I/O and limited write endurance. Its limitations notwithstanding, the overwhelming advantages offered by PCM indicate that it might not be long before database systems have PCM included in the underlying memory hardware. However, current query processing is designed keeping in mind the DRAM properties of read/write symmetry and unlimited write endurance. These assumptions being violated in a PCM environment, such design needs revisiting for suitability in the new setting. Earlier works on PCM friendly database algorithms targeted a myriad of possible PCM inclusive system models, without giving ample justification for the choice of their model. Moreover, they took individual operators in their ambit, regardless of their overall effect on query execution. We, for the first time in literature, delve into the pros and cons of all the existing system models. Subsequently, we propose PCM conscious algorithms for database ``workhorse" operators \textit{Sort}, \textit{Hash Join} and \textit{Group By}, relevant to the best model, in the context of full query execution. We include PCM support in a state-of-the-art simulator and use it to test our algorithms on TPCH benchmark queries. Our work shows an improvement of 30\% on writes and 20\% on run time, on average, as compared to existing algorithms; thus bringing databases a step closer to being amply prepared for the PCM transition.
\end{abstract}
...
%
\textit{Phase Change Memory (PCM)} is a type of non-volatile memory that is made up of chalcogenide glass which stores data by switching between crystalline and amorphous states. Not only is it 2-4X denser and lesser idle power consuming than DRAM , it also has a comparable read latency. Due to these properties, it has been abuzz in the computer architecture community as the next major memory technology potentially replacing or augmenting the current components of the memory hierarchy \cite{qureshi}, \cite{zhou}, \cite{lee}. In fact recently, IBM came up with a PCM chip that is 275 times faster than SSD \cite{ibm}.
...
The data explosion in this era of big data has ensured that DRAM capacities can no longer keep pace with database sizes. Consequently, most databases today are disk resident. Since majority of the tasks in database processing are data intensive, such a configuration amounts to CPU spending most of its time waiting for the data. Thus, PCM can be particularly beneficial to database systems to plug this latency gap between the DRAM and the disk. 

One peculiar characteristic of PCM is a significant difference between read and write behaviours in terms of energy, latency and bandwidth. Besides, it comes with a serious limitation of limited write endurance. Such issues with PCM need to be addressed before we can incorporate it in the memory hierarchy for mainstream data processing. Query execution in database systems has hitherto been designed assuming symmetric read and write behaviours. This assumption being violated in the PCM environment, current query processing algorithms can be grossly sub-optimal in terms of both time and writes in such a setting. The glaring performance advantage reads bestow us with when traded for writes cannot be overlooked. Hence, the design of query processing algorithms for PCM calls for a significant change in perspective as compared to current design methodology.  

There has been similar research undertaken earlier for database query processing on flash disks\cite{graefe}. However, PCM differs from flash in some key aspects. Firstly, flash supports block addressability whereas PCM is byte addressable. Secondly, the read latency of flash is about 32 times slower than DRAM whereas that of PCM is almost comparable. The techniques for flash devices were developed according to these block and latency properties. PCM, on the other had would demand a separate customized set of algorithms if it is to be utilized to its full potential.
...
\begin{itemize}
\item We scrutinize the various existing system models incorporating PCM in the memory hierarchy to suggest the best model.

\item We propose PCM conscious algorithms for \textit{sort}, \textit{hash join} and \textit{group by}, that constitute the ``workhorse" operators of database systems, in the context of end-to-end query execution.
...
There are four alternatives proposed to the current model when incorporating PCM in the memory hierarchy in the current literature \cite{qureshi}, \cite{lee}, \cite{condit}, \cite{viglas}. The corresponding arrangements are shown in Figure \ref{fig:pcm_models},

\begin{enumerate}[label=(\alph*)]

\item \textbf{PCM\_DISK}: In this model, PCM is proposed as an alternative to disk. The advantage of this model is that existing database systems can run oblivious to the change in the underlying hardware treating PCM as the disk. However, given the huge database sizes of today, the density and cost of PCM would make it to accommodate the entire database on PCM itself. Disk, by virtue of being much denser and cheaper, is likely continue to be the memory technology for persistent database storage.

\item \textbf{PCM\_DRAM}: This model corresponds to PCM entirely replacing the DRAM. Hence, the program address space now maps to PCM instead of the DRAM. It has the same advantage of straightforward portability of existing databases to new PCM inclusive systems, as the previous model. The limitation however is that since PCM now directly follows the Last Level Cache(LLC), each time there is an LLC miss, PCM latency will be incurred. Since PCM has relatively higher latency than DRAM, it will lead to significant degradation in the system performance.

\item \textbf{PCM\_DRAM\_EXPLICIT}: Both the DRAM and the PCM are in explicit control of the software in this model. Thus, there is great flexibility available in terms of leaving the utilization of the two available memory devices to the database or the operating system. 
This model would require putting in additional machinery in place in order to distinguish between data mapped to DRAM and the PCM; one such way being having separate address space mapping for both of them.  Such as organization would require the database programmer to have apriori information about the existence of PCM to utilize its availability. In the absence of such prior knowledge, such as in existing database systems, the onus of PCM utilization will be on the operating system designer in building within the OS additional intelligence of performing optimal data distribution on the DRAM and the PCM. The latter can be tricky since different applications have different data usage patterns. When none of the above is the case, databases would end up using only the the DRAM, and thus be devoid of the benefits of PCM availability.
...
\begin{algorithmic}[1]
\State $ k = c\times \frac{N_R L_R}{|DRAM|}$;
