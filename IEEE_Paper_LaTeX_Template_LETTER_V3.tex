...
%
\begin{abstract} 
\textit{Phase Change Memory (PCM)} is a non-volatile, random access and byte-addressable memory that is being touted as one of the most promising upcoming memory technologies. Besides its desirable features of high density, comparable read latency, and low idle power vis-a-vis the DRAM, PCM also comes with certain peculiar characteristics of asymmetric I/O and limited write endurance. Its limitations notwithstanding, the overwhelming advantages offered by PCM indicate that it might not be long before database systems have PCM included in the underlying memory hardware. However, current query execution is designed keeping in mind the DRAM properties of read/write symmetry and unlimited write endurance. These assumptions being violated in a PCM environment, such design choices need revisiting for suitability in the new setting. Prior work in this area targeted a myriad of PCM inclusive system models, without giving ample justification for the choice of their model. Moreover, they took individual operators in their ambit, regardless of their overall effect on query execution. We, for the first time in literature, present a critical evaluation of the existing system models from a database perspective. Subsequently, we propose PCM conscious algorithms for database ``workhorse" operators \textit{sort}, \textit{hash join} and \textit{group-by}, relevant to the best model, in the context of full query execution. We include PCM support in a state-of-the-art simulator and use it to test our algorithms on TPCH benchmark queries. Our work shows an improvement of X\% on writes and Y\% on run time, on average, as compared to existing algorithms; thus bringing databases a step closer to being amply prepared for the PCM transition.
\end{abstract}
...
There are multiple possible manners in which PCM can be incorporated in the memory hierarchy. Many diverse configurations already exist in the literature \cite{qureshi}, \cite{lee}, \cite{condit}, \cite{viglas}. Given the choice of the model heavily impacts the algorithm design choices, the need for a critical evaluation of existing system models to come up with the most potent candidate for the future is indispensable. Otherwise, all our efforts to come up with the right algorithm design may prove to be futile. 
...
\begin{itemize}
\item We scrutinize the various existing system models incorporating PCM in the memory hierarchy from a database perspective to come up with the best model.
...
\item We test our proposed algorithms on TPCH benchmark queries using the modified simulator. Our results show significant improvements both in running time and writes.
...
c) \textbf{DRAM\_EXPLICIT}: Both the DRAM and the PCM are in explicit control of the software in this model. Thus, there is great flexibility offered in terms of leaving the utilization of the two available memory devices to the database or the operating system. 
...
During a program run, three regions of memory are accessed - code, stack and the heap. All these regions would share a common DRAM. Hence, we need to account for the shortfall in DRAM space due to these additional components. If the DRAM were direct-mapped, then accessing one region may have lead to eviction of another. We assume a set-associative memory organization of DRAM with atleast 3-way set associativity. Assuming one way per set will be consumed by the stack data another way will be consumed by the executable code, the effective DRAM size (denoted by $|DRAM|$) would be $\frac{\#ways-2}{\#ways} \times Size_{DRAM}$. Note that for this to hold true, $\#sets$ should be greater than both the stack data size and the executable code size individually.
...
We propose the multi-pass group-by scheme, on the same lines as the multi-pass sort scheme, where we avoid materializing the partitions. We instead choose to scan through the entire set of tuples, ignoring the tuples not belonging to the current partition being processed. Algorithm   \ref{alg:non_materialized_partioned_gby} represents the pseudo code for for this variant.
